...
struct FreeWeb {
    ...
    budget: FrameBudget,
    local_backend: Option<net::local::LocalServer>,
}
...
impl FreeWeb {
    fn new(proxy: EventLoopProxy<UserEvent>, governor: perfmon::Governor) -> Self {
        ...
        Self {
            ...
            local_backend: net::local::LocalServer::spawn().ok(),
        }
    }
...
<p>Inline JavaScript runs in a sandbox. Pages that require browser DOM APIs,
cookies or advanced graphics may still be incomplete. Images show as alt-text
boxes. Forms do not submit yet.</p>
</body></html>"#;
...
fn script_sources(
    ...
    for id in d.iter() {
        let Some(node) = d.get(id) else { continue };
        let NodeType::Element(el) = &node.kind else { continue };
        if el.tag != "script" {
            continue;
        }
        if let Some(src) = el.get_attr("src") {
            ...
            if let Ok(response) = net::fetch_document(&url) {
                if response.status == 200 && response.body.len() <= 512 * 1024 {
        ...
        for child in &node.children {
            if let Some(NodeType::Text(text)) = d.get(*child).map(|n| &n.kind) {
...
/// linked stylesheets and sandboxed ECMAScript.
///
...
fn fetch_worker(
    ...
    let empty_dom_note = if response.body.len() < 256 * 1024 {
        ...
        if text_chars <= 80 {
            ...
            if lowered.contains("<script") {
                Some(
                    "Page loaded, but its content is mostly generated by JavaScript \
                     and may require DOM APIs not yet exposed"
                        .to_string(),
                )
            } else {
    ...
    let parse = std::panic::catch_unwind(std::panic::AssertUnwindSafe(move || {
        ...
        let script_report = script::execute_inline(&script_sources(&d, &base, &css_blocker));
        let css_src = collect_css(&d, &base, &css_blocker);
        let sheet = parse_stylesheet(&css_src);
        let title = d.title();
        let refresh = meta_refresh(&d, &base);
        (d, sheet, title, base, refresh, script_report)
    }));
    match parse {
        Ok((d, sheet, title, base, refresh, script_report)) => {
            ...
            if script_report.executed > 0 {
                let note = format!(
                    "JavaScript ejecutado: {} script(s), {} con error",
                    script_report.executed, script_report.failed
                );
                status_error = Some(match status_error {
                    Some(prev) => format!("{prev} | {note}"),
                    None => note,
